...
@Test
func tastyTradeAdapterAccounts() async throws {
  let configuration: URLSessionConfiguration = .ephemeral
  configuration.protocolClasses = [TastyTradeURLProtocol.self]

  let json: Data = try Bundle.module.json(named: "accounts")
  TastyTradeURLProtocol.handler = { (_: URLRequest) in
    let url: URL =
  ...
  let service = TastyTradeAccountService(client: .init(configuration: configuration))
  let numbers = try await service.accountNumbers()
  #expect(numbers == ["123", "456"])
...
final class TastyTradeURLProtocol: URLProtocol {
