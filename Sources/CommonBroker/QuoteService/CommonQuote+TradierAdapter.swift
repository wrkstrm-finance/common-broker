...
extension CommonQuote {
  ...
  public init(_ tradierQuote: Tradier.Quote) {
    ...
    tradeDate = secondsOrMillisToDate(tradierQuote.tradeDate)
    ...
    bidsize = Int(tradierQuote.bidsize * 100)  // SDK uses 100s; normalize to shares
    bidexch = tradierQuote.bidexch
    bidDate = secondsOrMillisToDate(tradierQuote.bidDate)

    ask = tradierQuote.ask
    asksize = Int(tradierQuote.asksize * 100)
    askexch = tradierQuote.askexch
    askDate = secondsOrMillisToDate(tradierQuote.askDate)
    ...
    expirationDate = parseExpiration(tradierQuote.expirationDate)
