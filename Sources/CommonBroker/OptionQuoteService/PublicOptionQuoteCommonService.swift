...
public struct PublicOptionQuoteCommonService: CommonOptionQuoteService, Sendable {
  ...
  public func optionQuote(for symbol: String, accountId: String) async throws -> CommonOptionQuote {
    ...
    async let greeksTask: PublicClient.OptionGreeks = client.fetchOptionGreeks(
      for: accountId, symbol: symbol,
    )

    let quotes = try await quotesTask
    guard let first = quotes.first else { throw StringError("No option quote for \(symbol)") }
